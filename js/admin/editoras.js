...
function showModal(publisher) {
  ...
  document
    .querySelector(".publisher-modal")
    .classList.toggle("editing", Boolean(publisher));
...
function renderPagination(totalPublishers, firstPublisher) {
  const totalPages = Math.max(
    1,
    Math.ceil(totalPublishers / PUBLISHERS_PER_PAGE),
  );
  const lastPublisher = Math.min(
    firstPublisher + PUBLISHERS_PER_PAGE,
    totalPublishers,
  );
...
function renderPublishers() {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredPublishers.length / PUBLISHERS_PER_PAGE),
  );
  currentPage = Math.min(currentPage, totalPages);
  const firstPublisher = (currentPage - 1) * PUBLISHERS_PER_PAGE;
  const pagePublishers = filteredPublishers.slice(
    firstPublisher,
    firstPublisher + PUBLISHERS_PER_PAGE,
  );
...
form.addEventListener("submit", (event) => {
  ...
  const emailExists = publishers.some(
    (publisher) =>
  ...
  const savePublisher = () => {
    ...
    if (editingPublisherId) {
      const publisher = publishers.find(
        (item) => item.id === editingPublisherId,
      );
  ...
  if (editingPublisherId) {
    askConfirmation(
      "Você tem certeza que deseja realizar essas alterações?",
      savePublisher,
    );
  } else {
    savePublisher();
...
deleteButton.addEventListener("click", () => {
  ...
  askConfirmation("Você tem certeza que deseja excluir todos os dados?", () => {
    ...
    const relatedBooks = books.some(
      (book) => book.publisher === publisher?.name,
    );
    if (relatedBooks) {
      alert(
        "Não é possível excluir esta editora, pois existem livros relacionados a ela.",
      );
      return;
    } else {
      publishers = publishers.filter(
        (publisher) => publisher.id !== editingPublisherId,
      );
      savePublishers();
      renderPublishers();
      hideModal();
