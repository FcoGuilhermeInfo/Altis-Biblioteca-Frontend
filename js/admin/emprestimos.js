...
function renderCurrentLoans() {
  ...
  const activeLoans = loans.filter(
    (loan) =>
      !loan.returnedAt &&
      getUserName(loan.userId).toLowerCase().includes(search),
  );
  const totalPages = Math.max(
    1,
    Math.ceil(activeLoans.length / LOANS_PER_PAGE),
  );
...
document.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && fullHistoryOverlay.classList.contains("open"))
    fullHistoryClose.click();
});
...
form.onsubmit = (event) => {
  ...
  const user = users.find(
    (item) =>
      item.name.toLowerCase() === form.usuario.value.trim().toLowerCase(),
  );
  const book = books.find(
    (item) =>
      item.title.toLowerCase() === form.livro.value.trim().toLowerCase(),
  );
  ...
  if (!startDate || !dueDate || dueDate < startDate)
    return alert("Informe datas válidas.");
  if ((book.total || 0) - (book.borrowed || 0) < 1)
...
returnButton.onclick = () => {
  ...
  if (!loan) return;
  askConfirmation(
    "Confirma a devolução do livro por parte do locatário?",
    () => {
