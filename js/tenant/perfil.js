...
const user = users.find(item =>
  item.id === loggedUser?.id || item.email === loggedUser?.email
);
...
function saveProfile() {
  ...
  const userIndex = users.findIndex(item => item.id === user.id || item.email === user.email);
  users[userIndex] = user;
  localStorage.setItem('locbooksUsers', JSON.stringify(users));
  localStorage.setItem('locbooksLoggedUser', JSON.stringify({ ...loggedUser, ...user }));
  renderProfile();
  closeConfirmation();
  closeEditModal();
}
...
editForm.addEventListener('submit', event => {
  ...
  const hasPasswordChange = editForm.senhaAtual.value || editForm.novaSenha.value || editForm.confirmarSenha.value;
