...
function getMonthlyRanking() {
  const totals = {};
  loans
    .filter((loan) => isThisMonth(loan.startDate))
    .forEach((loan) => {
...
function renderLatestLoan() {
  ...
  document.querySelector(".latest-book-author").textContent = getUserName(
    latestLoan.userId,
  );
  document.querySelector(".latest-loan-date").textContent = formatDate(
    latestLoan.startDate,
  );
  document.querySelector(".latest-due-date").textContent = formatDate(
    latestLoan.dueDate,
  );
}
...
function renderPopularBook(ranking) {
  ...
  document.querySelector(".popular-book-title").textContent =
    mostPopular.book.title;
  document.querySelector(".popular-book-author").textContent =
    mostPopular.book.author || "";
