...
if (form) {
  form.addEventListener('submit', function (event) {
    ...
    const isAdmin = typedEmail === ADMIN.email && typedPassword === ADMIN.password;
    const user = users.find(function (item) {
      return (item.email || '').toLowerCase() === typedEmail && item.password === typedPassword;
    });
    ...
    const loggedUser = isAdmin ? { ...ADMIN, role: 'admin' } : { ...user, role: 'locatario' };
