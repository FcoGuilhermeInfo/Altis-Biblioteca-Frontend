...
form.onsubmit = (event) => {
  ...
  const bookData = {
    ...
    year: form.ano.value,
    publisher: form.editora.value.trim(),
    total: Number(form.quantidade.value),
  };
...
deleteButton.onclick = () => {
  ...
  askConfirmation("Você tem certeza que deseja excluir todos os dados?", () => {
    ...
    const relatedLoans = loans.some(
      (loan) => loan.bookId === editingBookId && !loan.returned,
    );
    if (relatedLoans) {
      alert(
        "Não é possível excluir este livro, pois existem empréstimos relacionados a ele.",
      );
      return;
    } else {
